...
def train(trainset, validset, testset, run_tag, hp):
    ...
    if hp.device == 'cpu':
        if torch.cuda.is_available():
            print("Warning: using CPU, but CUDA is available")
    if hp.device == 'cuda' and not torch.cuda.is_available():
        print("Warning: using CPU, but CUDA is not available")
        hp.device = 'cpu'


    model = DittoModel(device=hp.device,
                       lm=hp.lm,
    ...
    # Native PyTorch AMP: create scaler if using fp16
    scaler = GradScaler() if hp.fp16 and hp.device == 'cuda' else None
